...
public class DBReservation {
    ...
    public void createTable() {
        try {
            String query = "create table if not exists Reservation ( 	Id INT AUTO_INCREMENT,	ShowId INT,	UserId INT,	MovieName VARCHAR(30),	Date DATE,	TotalPrice DOUBLE, PRIMARY KEY(Id) )";
    ...
    public Reservation insertObject(Reservation p) {
        try {
            String query = "insert into Reservation( 	ShowId ,	UserId ,	MovieName ,	Date ,	TotalPrice ) VALUES (?,?,?,?,?)";
            PreparedStatement st = conn.prepareStatement(query);
    ...
    public boolean updateObject(Object o) {
        try {
            Reservation p = (Reservation) o;
            String query = "update  Reservation set 	ShowId=? ,	UserId=? ,	MovieName=? ,	Date=? ,	TotalPrice=? where Id = ?";
            PreparedStatement st = conn.prepareStatement(query);
            st.setInt(1, p.getShowId());
            st.setInt(2, p.getUserId());
            st.setString(3, p.getMovieName());
